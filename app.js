 ...
 *   • Save & Load timetables  (localStorage)
 *   • UI rendering and event wiring
 * ─────────────────────────────────────────────────────────
 */

/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   0. PDF.js worker setup
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
...
const THEME_ICONS = { light:"☀️", dark:"🌙", ocean:"🌊", forest:"🌿", sunset:"🌅" };
...
const TIME_SLOTS = ["6:00 PM \u2013 7:00 PM","7:00 PM \u2013 8:00 PM","8:00 PM \u2013 9:00 PM"];
...
function ensureAllPresent(schedule, subjects) {
  const seen = new Set(schedule.flat().filter(Boolean).map((s) => s.name));
...
function generateTimetable(subjects) {
  ...
  for (let d = 0; d < DAYS.length; d++) {
    ...
    for (let s = 0; s < maxSlots; s++) {
      ...
      const pick = shuffle(subjects).find((sub) => {
        ...
        if (prev?.difficulty === "hard" && sub.difficulty === "hard") return false;
        return true;
      });
      if (pick) { schedule[d][s] = pick; used.add(pick.name); count[pick.name]++; }
    }
  }
  ...
  return DAYS.map((day, d) => ({
    ...
    slots: schedule[d]
      .map((sub, s) => sub
        ? { subject: sub.name, difficulty: sub.difficulty, timeSlot: TIME_SLOTS[s] }
        : null)
      .filter(Boolean),
  }));
}
...
/** @returns {{ id:string, name:string, savedAt:number, subjects:object[], timetable:object[] }[]} */
function getSaves() {
...
function showToast(msg, type = "info") {
  ...
  Object.assign(t.style, {
    ...
    color:"#fff", padding:".55rem 1.25rem", borderRadius:"10px",
    fontSize:".88rem", fontWeight:"600", zIndex:"9999",
    boxShadow:"0 4px 20px rgba(0,0,0,.3)", opacity:"1",
    transition:"opacity .35s", whiteSpace:"nowrap",
  });
  document.body.appendChild(t);
  setTimeout(() => { t.style.opacity = "0"; setTimeout(() => t.remove(), 380); }, 2600);
}
...
function goToStep(n) {
  [1, 2, 3].forEach((i) => {
    ...
    if (dot) dot.className = "step " + (i < n ? "done" : i === n ? "active" : "inactive");
  });
...
function renderSubjectList() {
  ...
  subjects.forEach((sub, idx) => {
    ...
    inp.value = sub.name;
    inp.addEventListener("input", () => { subjects[idx].name = inp.value; });
    ...
    del.addEventListener("click", () => { subjects.splice(idx, 1); renderSubjectList(); });
    li.appendChild(inp);
    li.appendChild(del);
    list.appendChild(li);
  });
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   8. Render: Step 2 difficulty picker
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   9. Render: Step 3 timetable grid
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
...
function renderSavesModal() {
  ...
  saves.forEach((save) => {
    ...
    // Show subject count + date
    const subjectNames = save.subjects.map((s) => s.name).slice(0, 3).join(", ");
    const more         = save.subjects.length > 3 ? ` +${save.subjects.length - 3} more` : "";
...
function init() {
  ...
  zone.addEventListener("drop",      (e) => { e.preventDefault(); zone.style.borderColor = ""; handlePdf(e.dataTransfer.files[0]); });
  input.addEventListener("change",   ()  => { handlePdf(input.files[0]); input.value = ""; });
  ...
  async function handlePdf(file) {
    ...
    try {
      ...
      renderSubjectList();
      showToast(`${newOnes.length} subject${newOnes.length !== 1 ? "s" : ""} extracted.`);
    } catch (err) {
      console.error(err);
  ...
  document.getElementById("open-saves-btn").addEventListener("click", openSavesModal);
  ...
  document.getElementById("next-to-2").addEventListener("click", () => {
    ...
    if (!valid.length) { showToast("Add at least one subject.", "error"); return; }
    subjects = valid;
  ...
  function runGenerate() {
    if (subjects.some((s) => !s.difficulty)) {
      showToast("Assign a difficulty to every subject first.", "error"); return;
    }
